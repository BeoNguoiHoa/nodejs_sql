...
app.post("/luu-khach-hang",function (req, res) {
    ...
    let sql_text = "INSERT INTO Lab4_KhachHang(TenKH,DienThoai,DiaChi) VALUES('"+ten+"','"+dt+"','"+dc+"')";
...
app.post("/luu-san-pham",function (req, res) {
    ...
    let sql_text = "INSERT INTO Lab4_SanPham(TenSP,MoTa,DonVi,Gia) VALUES('"+tensp+"','"+mota+"','"+donvi+"','"+gia+"')";
    db.query(sql_text,function (err,rows) {
        if(err) res.send(err);
...
app.post("/luu-don-hang",function (req,res) {
    ...
    db.query(sql_text,function (err,rows) {
        if(err) res.send(err)
            ...
            let sql_text = "INSERT INTO Lab4_DonHang(KhachHangID,TongTien,ThoiGian) VALUES("+khID+","+tongtien+",GETDATE());SELECT SCOPE_IDENTITY() AS MaSo;"
            db.query(sql_text,function (err,rows) {
                let donhang = rows.recordset[0];
                let MaSo = donghang.MaSo
                let sql_text3 = "";
                sps.map(function (e) {
                ...
                db.query(sql_text3,function (err,rows) {
                    if(err) res.send(err);
                    else res.send("Tao don hang thanh cong!");
                })
            })
        }
    })
...
app.get("/chi-tiet-khach-hang/:id",async function (req,res) {
    ...
    let sql_text2 = "SELECT * FROM Lab4_KhachHang WHERE KhachHangID ="+khid;
    let donhang = [];
